...
The default output is a complete storyboard in clean Markdown. If the user explicitly requests a .docx file and the current Goose environment has an available document-generation capability, produce a .docx version that the SME can review, edit, and hand off to a learning designer for development.


Who This Is For
Primary users: Compliance subject matter experts (SMEs) who are knowledgeable in regulations but not trained in instructional design. Their input: Draft compliance content — most commonly a structured document or slide deck (e.g., Google Docs or Google Slides) — that is accurate but not yet designed for learning. Other formats are supported. Their output: A complete storyboard they can review, refine, and hand off to a learning designer for development.


Core Design Principles
Every storyboard must reflect these four principles:

User-Centricity — Content is clear, intuitive, and easy to navigate. Learners can quickly understand what they need to know and do. Avoid jargon where possible and prioritise helping the learner take action.
Engaging Storytelling — Training reads as a cohesive, purposeful narrative — not a list of rules. Provide context, highlight why the content matters, and drive action. The "why" should always be visible.
On-Brand Relevance — Examples and scenarios should feel aligned to the organisation, its products, and its compliance priorities. When possible, situations should reflect realistic, role-relevant workplace contexts drawn directly from the source content.
Evolving with Agility — Structure should be modular and easy to update. Each section should be independently revisable as regulations, policies, or priorities change over time.
...
Use references/hr-workplace.md only when the source content is about HR, workplace conduct, harassment, retaliation, accommodation, workplace safety, or similar employee-compliance topics.
For non-HR compliance domains, rely on the user-provided source content and do not import HR/workplace examples, scenarios, or terminology unless the user explicitly asks for them.
Supporting references are optional aids. They must never override the user's source material or introduce requirements that are not present in the source.
...
Apply these standards whenever producing a formatted document, including .docx output when available.


Output Format and Cross-Platform Document Generation
Default to producing the complete storyboard in Markdown unless the user explicitly asks for a .docx file.

If .docx generation is requested:

Use an available document-generation skill, extension, or local script if one is present in the current Goose environment.
Do not reference absolute, platform-specific paths such as /mnt/skills/public/docx/SKILL.md.
Do not assume a separate docx skill exists unless it is actually available in the current environment.
If no .docx generation capability is available, provide the storyboard in clean Markdown and tell the user that a document-generation tool or skill is needed to convert it to .docx.
Any generated .docx must follow the Document Formatting Standards above.
...
Verbs to Use
1 – Remember
identify, recall, list, name, define
2 – Understand
explain, describe, summarise, classify
3 – Apply
apply, use, demonstrate, solve, complete
4 – Analyse
distinguish, compare, differentiate, examine
...
Introduction (video)
2–3 minutes
Each core concept section
5–8 minutes
Each scenario
2–4 minutes
Each knowledge check question
~30 seconds
Summary
2–3 minutes
